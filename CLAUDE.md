...
- **/reports** — «**Выгрузка УТКО**» (сайдбар, icon download): история сформированных .xlsx-выгрузок
  ...
  нет) · Описание(comment) · Ссылка на фото ×6 (ПРОСТО текстовые полные URL, без встраивания). Формирование
  переводит включённые обращения в статус `exported`. Файл: `Выгрузка_УТКО_<дата>.xlsx`.
