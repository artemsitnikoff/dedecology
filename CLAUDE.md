...
- **/reports** — «**Выгрузка УТКО**» (сайдбар, icon download): история сформированных .xlsx-выгрузок
  ...
  НАШЕГО справочника `Region.name` синхр. из УТКО — `mno_id→Mno.region_code→Region.name`, фолбэк inc.region;
  НЕ DaData-текст) · Реестровый № МНО · Дата+время фотофиксации · Адрес(город+улица) · Тип инцидента · Подтип(пусто, поля
