...
- **incidents:** id, source `max|form|app` (`app` — мобильное приложение волонтёра: тот же POST
  ...
  сортировки/периода), photos (1..6 — было 3), photo_urls (JSONB list), msg (id сообщения Макса, nullable),
...
- **/reports** — «**Выгрузка УТКО**» (сайдбар, icon download): история сформированных .xlsx-выгрузок
  (таблица: дата+время · строк · размер · сформировал · скачать/удалить), серверная пагинация `?rpage`,
  пустое состояние 💚. Формируются со страницы «Инциденты» кнопкой «**Выгрузить в УТКО**» (шапка — по
  фильтру, bulk — по выбранным); прямого скачивания со страницы нет — файл уходит в раздел. **Формат
  файла — УТКО** (`services/utko_export.py`, НЕ обычный `export.py`): плоская таблица Субъект РФ (из
  НАШЕГО справочника `Region.name` синхр. из УТКО, НЕ DaData-текст — иначе УТКО отвергнет строку: есть МНО →
  `mno_id→Mno.region_code→Region.name`; НЕТ МНО → резолв текста `inc.region` по справочнику
  (`region.canonical_index` + `addr_norm.region_match_key`: регистр/ё/тире/ведущий «г.» → «Санкт-Петербург»
  даёт канон «г. Санкт-Петербург»); нет в справочнике → `inc.region` как есть) · Реестровый № МНО · Дата+время фотофиксации · Адрес(город+улица) · Тип инцидента · Подтип(пусто, поля
  нет) · Описание(comment) · Ссылка на фото ×6 (ПРОСТО текстовые полные URL, без встраивания; ведут на
  **УТКО-копию** `{i}_utko.jpg` — ФГИС УТКО отвергает фото >500 КБ, поэтому под выгрузку `intake._store_photos`
  пишет отдельную «тощую» копию ≤1280×720 и ≤500000 байт; FULL `{i}.jpg` для админки не трогается; `get_photo`
  генерит копию лениво из FULL при первом запросе, старые фото прогреваются `python -m app.backfill_utko_photos`).
  Формирование переводит включённые обращения в статус `exported`. Файл: `Выгрузка_УТКО_<дата>.xlsx`.
...
```bash
cd backend
.venv/bin/python -m pytest -q          # тесты (оффлайн, без внешней БД) — сейчас 716/716 зелёные
...
- **Хранилище файлов (фото/отчёты/логи) — ФАЙЛОВОЕ, S3 в коде НЕТ.** Всё под `STORAGE_DIR`
  ...
  миниатюры в .xlsx с диска. **Нативный S3 в коде пока не реализован (в планах); до него — монтирование бакета (ниже).**
...
**Раздел «Новые МНО» (`/mno-new`):** волонтёрские МНО (`Mno.source='volunteer'`) вынесены в отдельный
раздел — тот же компонент `MnoPage` с пропом `sourceFilter` (`/mno` → `'fgis'` без волонтёрских, `/mno-new`
→ `'volunteer'`). GET /mno/(points) получил фильтр `source`. В «Новых МНО» — колонка/поле «Дата создания»
(= `Mno.created_at`, TimestampMixin, фиксируется на вставке; в API отдаётся как **`MnoListItem.received_at`**
— имя под мобильное приложение), кнопки ФГИС-синка/ручного добавления скрыты, столбец «Реестровый №»
скрыт (у волонтёрских его нет). **Фото обращения — до 6** (было 3): intake `_MAX_PHOTOS=6`, форма, maxbot
(`_all_images`/notify), экспорт build_xlsx «Фото 1..6» (УТКО — уже 6).
