...
- Миграции: `0001_initial` (сначала `CREATE EXTENSION pgcrypto`) · `0002` notified_at+quote ·
  ...
  `0006` regions+mno · `0007` индекс `ix_mno_fgis_id` (upsert МНО по fgis_id) · `0008` mno.address→TEXT · `0009` lat/lon+индекс · `0010` incidents.incident_type · `0011` таблица incident_types (редактируемый справочник).
...
```bash
cd backend
.venv/bin/python -m pytest -q          # тесты (оффлайн, без внешней БД) — сейчас 346/346 зелёные
