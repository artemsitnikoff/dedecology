...
- **/reports** — «**Выгрузка УТКО**» (сайдбар, icon download): история сформированных .xlsx-выгрузок
  ...
  файла — УТКО** (`services/utko_export.py`, НЕ обычный `export.py`): плоская таблица Субъект РФ ·
  Реестровый № МНО · Дата+время фотофиксации · Адрес(город+улица) · Тип инцидента · Подтип(пусто, поля
