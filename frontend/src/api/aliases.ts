...
/** Источник обращения. */
export type Source = 'max' | 'form' | 'app' | 'telegram';
...
/**
 * Историческая подпись для кодов типа инцидента, УДАЛЁННЫХ из справочника incident_types,
 * но всё ещё встречающихся у СУЩЕСТВУЮЩИХ обращений (Incident.incident_type хранит код,
 * а не подпись). code='other' («Иное») выпилен из БД-справочника — новые обращения его
 * больше не получат (в дропдаунах его нет — они строятся из useIncidentTypes()/API), но
 * старые обращения с этим кодом должны по-прежнему показывать русскую подпись, а не «other».
 * Подставляется ТОЛЬКО как фолбэк — если код есть в живом справочнике, побеждает его подпись.
 */
...
export interface MnoListItem {
  ...
  fgis_id: string | null;
  /**
   * Происхождение записи: 'fgis' — из ФГИС/добавлено вручную экологом (по умолчанию);
   * 'volunteer' — добавлено волонтёром на публичной форме (эколог проверяет — показываем бейдж).
   */
  source: string;
