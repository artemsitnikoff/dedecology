"""Стейт-машина интерактивного приёма МНО в личке Telegram-бота.

ЧИСТАЯ логика БЕЗ импорта Telegram-клиента — юнит-тестируется оффлайн (см.
`telegrambot/tests/test_session.py`). Перенесена дословно из maxbot/session.py
(с добавленным флагом `awaiting_location` под гео-шаг). Обращение в личке
создаётся ТОЛЬКО после выбора площадки (кнопка МНО / «Нет в списке»), поэтому
между `prepare` и `finalize` нужно где-то держать: скачанные фото, разобранные
поля адреса и список кандидатов. Этим занимается `PendingReport` + `PendingStore`.

Хранилище — процессное (in-memory). Telegram-бот запускается ОДНИМ long-polling
процессом (`telegrambot/main.py` → getUpdates-цикл), поэтому единого стора
...
# Префикс payload callback-кнопок. callback_data Telegram ограничен 64 БАЙТАМИ;
# наш формат «m:{uuid4-hex}:{idx}» ~38 символов — с запасом.
PAYLOAD_PREFIX = "m"
...
# Максимальная длина текста инлайн-кнопки (держим короче для читаемости в узком
# столбце клавиатуры; Telegram допускает длиннее).
BUTTON_MAX = 40
...
def chat_key(chat_id, user_id) -> str:
    """Ключ диалога «chat_id:user_id» — стабилен между message-апдейтом
    (message.chat.id / message.from.id) и callback_query-апдейтом
...
def button_text(i: int, reg: str = "", name: str = "") -> str:
    ...
    Реестровый № (reg, напр. «78-06-002210») приоритетнее названия — короткий и однозначно
...
class PendingReport:
    ...
    # id сообщения с картой OSM (метки 1/2/3) — чтобы удалить его при выборе площадки.
    ...
    # Ждём от пользователя геопозицию (кнопка «📍 Отправить геопозицию») — гео-шаг
    # Telegram-бота: после присланного фото до отправки координат/адреса. Отдельно от
    # awaiting_address, чтобы отличать фазу гео-кнопки (в maxbot этого шага нет).
    awaiting_location: bool = False
    # Ждём от пользователя адрес текстом (координаты не распознались из подписи, либо
    # текстовый фолбэк вместо гео-кнопки).
...
class PendingStore:
    ...
    def find_awaiting(self, key: str, now: float | None = None) -> PendingReport | None:
        ...
        геопозицию ИЛИ адрес. При нескольких — вернуть самое свежее (latest created_at).

        Гео-шаг Telegram-бота и текстовый фолбэк живут в одном ожидании: пока обращение
        держит awaiting_location или awaiting_address, оно принимает и location-, и
        текст-апдейт (адрес). Финализированные/чужие/протухшие — пропускаются."""
        best: PendingReport | None = None
        for r in self._items.values():
            if r.finalized or not (r.awaiting_location or r.awaiting_address):
                continue
            if r.key != key:
                continue
