"""Юнит-тесты чистой логики session.py (БЕЗ импорта Telegram-клиента).

Покрывают контракт: payload roundtrip, TTL-purge, обрезку кнопок, map_query,
merge_parsed и find_awaiting (в т.ч. гео-ожидание awaiting_location). Перенесены
из maxbot/tests/test_session.py + дополнены случаем awaiting_location.
"""

from __future__ import annotations

from telegrambot.session import (
    BUTTON_MAX,
    NO_MNO,
    PENDING_TTL,
    PendingReport,
...
def test_payload_fits_telegram_callback_data_limit():
    # callback_data Telegram ограничен 64 БАЙТАМИ — наш payload обязан влезать.
    p = encode_payload(new_pending_id(), 5)
    assert len(p.encode("utf-8")) <= 64
    t = encode_type_payload(new_pending_id(), "no_access_to_mno")
    assert len(t.encode("utf-8")) <= 64
    s = encode_subtype_payload(new_pending_id(), "blocked_by_car")
    assert len(s.encode("utf-8")) <= 64
...
def test_find_awaiting_matches_awaiting_location():
    # Гео-шаг Telegram-бота: отчёт с awaiting_location (но без awaiting_address) тоже
    # находится find_awaiting — он принимает и location-, и текст-апдейт.
    store = PendingStore(ttl=PENDING_TTL)
    now = 10_000.0
    key = chat_key(1, 2)
    r = _report("geo", created_at=now - 5, awaiting=False, awaiting_location=True)
    store.put(r)
    assert store.find_awaiting(key, now=now) is r
