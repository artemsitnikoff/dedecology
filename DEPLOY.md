...
```bash
cd /opt            # или /srv, /var/www — куда удобно
...
**Настроить один раз:**
1. Сначала пройди ПЕРВЫЙ деплой вручную (§1–§5): репо склонировано в `/opt/dedecology`, `.env`
...
3. GitHub → repo **Settings → Secrets and variables → Actions → New repository secret**:
   - `SSH_HOST` = IP сервера
   - `SSH_USER` = пользователь (владелец `/opt/dedecology`, с доступом к docker)
...
4. Клонировал не в `/opt/dedecology`? Поправь путь `cd ...` в `deploy.yml`.
