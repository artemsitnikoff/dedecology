...
Авторизация — долгоживущий OAuth-токен CLAUDE_CODE_OAUTH_TOKEN (из `claude setup-token`).
...
def resolve_claude_token() -> str:
    """access_token для claude CLI — из CLAUDE_CODE_OAUTH_TOKEN (env). Пусто →
    генерация не выполняется (вызывающий деградирует на фолбэк)."""
...
async def claude_cli_complete(
    *,
    ...
    if not token:
        logger.info("[claude_cli] CLAUDE_CODE_OAUTH_TOKEN пуст — генерация пропущена")
        return None
