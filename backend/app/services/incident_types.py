...
INCIDENT_TYPES: list[dict] = [
    ...
    # 'other' («Иное») УБРАН из рантайм-справочника (БД-таблица incident_types, миграция
    # 0029) — в новых обращениях (форма/боты/админка) его выбрать НЕЛЬЗЯ. Здесь оставлен
    # НА МЕСТЕ ИСКЛЮЧИТЕЛЬНО как статик-фолбэк подписи для ИСТОРИЧЕСКИХ инцидентов с уже
    # проставленным кодом 'other': incident_type_label('other') → «Иное» (utko_export.
    # _type_label / export.py падают сюда, когда кода нет в БД-labels). НЕ удалять.
