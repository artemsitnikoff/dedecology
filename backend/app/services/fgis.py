...
START_Z = 4
# Максимальный зум дробления. На z16 даже плотные города рассыпаются в одиночки (замер по
# Новосибирску: макс. кластер ~21 < 100) → обрезки на MAX_Z практически нет. Глубже бурятся
# ТОЛЬКО кластеры, которые на z14 ещё >100 (единичные сверхплотные точки) — остальное
# завершается раньше (кластер <=100 → берётся целиком, дальше не дробится).
MAX_Z = 16
CLUSTER_LIMIT = 100        # массив ids в кластере обрезан на 100 (жёсткий предел API ФГИС)
# Гард ТОЛЬКО против рантайм-runaway (напр. баг). НЕ штатный стоп: обход завершается САМ,
# когда очередь пуста. Глубина ограничена MAX_Z → всё дерево обхода конечно (теор. максимум
# ~21 800 тайлов при MAX_Z=16, 4 START-ячейки). Лимит заведомо ВЫШЕ него → не срезает краул.
TILE_REQUEST_LIMIT = 100000
