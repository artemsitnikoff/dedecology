...
S3: нативную интеграцию с S3 в коде мы пока не успели сделать (в планах). Рабочее решение
без доработки — смонтировать S3-бакет в подкаталоги storage/incidents и storage/mno
(rclone mount) и распаковать архив в них: фото уедут в S3 прозрачно, приложение менять не
нужно. Полная процедура (rclone config -> systemd -> bind-mount в compose) и переменные
S3_* — в репозитории: DEPLOY.md §11.
