// Боковая панель
function Sidebar({ total, mnoCount, regionsCount, view, onNav }) {
  ...
  return (
    <aside style={{ width: 248, flex: 'none', background: '#F4F6F8', borderRight: '1px solid #E6E9EC', display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div style={{ padding: '12px 10px', display: 'flex', flexDirection: 'column', gap: 2, flex: 1, minHeight: 0, overflowY: 'auto' }} className="de-scroll">
        ...
        <button className="de-navrow" onClick={() => onNav('mno')} style={navStyle(view === 'mno')}>
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round"><path d="M21 10c0 6.5-9 12-9 12s-9-5.5-9-12a9 9 0 0 1 18 0z" /><circle cx="12" cy="10" r="2.6" /></svg>
          <span style={{ flex: 1 }}>МНО</span>
          <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 11, fontWeight: 600, background: '#E7F7ED', color: '#128640', padding: '1px 7px', borderRadius: 999 }}>{mnoCount}</span>
        </button>

        <div style={{ margin: '14px 11px 4px', fontSize: 10.5, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.06em', color: '#AEB6C0' }}>Справочники</div>
        <button className="de-navrow" onClick={() => onNav('regions')} style={navStyle(view === 'regions')}>
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round"><path d="M9 4 3.6 5.8a1 1 0 0 0-.6.9V19a1 1 0 0 0 1.3 1L9 18.5l6 2 4.7-1.8a1 1 0 0 0 .6-.9V5a1 1 0 0 0-1.3-1L15 5.5z" /><path d="M9 4v14.5M15 5.5V20" /></svg>
          <span style={{ flex: 1 }}>Регионы</span>
          <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 11, fontWeight: 600, background: '#F4F6F8', color: '#5B6573', padding: '1px 7px', borderRadius: 999 }}>{regionsCount}</span>
        </button>
