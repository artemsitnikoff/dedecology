...
**Мои отчёты** — инциденты, созданные ЭТИМ волонтёром из приложения (для таба «Отчёты»). **Bearer (volunteer JWT).**
- **Query:** `page` (≥1, деф. 1), `page_size` (1..200, деф. 50).
- **200:** `{ "items": [ IncidentListItem ], "total": N, "page": …, "page_size": …, "pages": … }` —
  свежие первыми (по дате создания). Элемент несёт **`status`** (`new|found|none|exported`), адрес
  (`region/city/street/coords`), `incident_type`, `comment`, `photo_time`, `photos`, `photo_urls`, `received_at`.
- ⚠️ Показываются только отчёты, где при создании было **застолблено авторство** — т.е. `POST /intake/form`
  вызывался **с заголовком `Authorization: Bearer <volunteer JWT>`** (см. §6). Анонимные (без токена) сюда не попадают.

### GET /volunteer/mno
**Мои МНО** — площадки, добавленные ЭТИМ волонтёром из приложения. **Bearer (volunteer JWT).**
- **Query:** `page` (≥1, деф. 1), `page_size` (1..200, деф. 50).
- **200:** `{ "items": [ MnoDetail ], "total": N, … }` — свежие первыми. Элемент — карточка МНО
  (`id, name, address, city, coords, source:"volunteer", comment, photo_urls, incidents, …`).
- ⚠️ Только МНО, созданные **с заголовком `Authorization`** на `POST /intake/mno` (см. §7). Анонимные не попадают.
...
элементе — используются для отрисовки на карте.

> **Авторизация:** ЧТЕНИЕ МНО — `GET /mno`, `GET /mno/points`, `GET /mno/{id}` — доступно
> **и админ-JWT, и волонтёрскому JWT** (`typ=volunteer`), чтобы приложение волонтёра показывало
> площадки на карте. Запись (создание/синхронизация/экспорт) — только админ. Волонтёру для
> подбора площадки на форме удобнее лёгкий публичный `GET /intake/mno-points?bbox=` (без токена).
...
- 🔑 **Авторство (для таба «Мои отчёты»):** приложение ДОЛЖНО присылать заголовок
  `Authorization: Bearer <volunteer JWT>`. Тогда отчёт привязывается к волонтёру и
  попадёт в `GET /volunteer/reports`. Токен **опционален** (без него — анонимный приём,
  как раньше, но отчёт в «Мои» не попадёт). Битый/чужой токен просто игнорируется.
- **Поля формы:**
...
| `mno_id` | text | нет | UUID выбранного/добавленного МНО (из `GET /intake/mno-points` или `POST /intake/mno`) — ссылка на объект ТКО |
| `mno_reg` | text | нет | рег-номер того же МНО (для показа) |
...
### GET /intake/mno-points
Точки МНО для выбора площадки на карте в форме. **Публичный.** `bbox` обязателен.
- **Query:** `bbox=minLat,minLon,maxLat,maxLon` (кадр карты/гео) → только МНО этого кадра (кап 500).
- **200:** `{ "points": [{ "id","coords","reg","address","region","city","name" }], "total": N, "capped": bool }`.
- Клик по точке подставляет в отчёт: `mno_id`=`id`, `mno_reg`=`reg`, а также регион/город/улицу/координаты.

### POST /intake/mno
Добавить МНО, **если нужного нет на карте** (волонтёр создаёт свой). **Публичный** (honeypot `website`).
- 🔑 **Авторство (для таба «Мои МНО»):** присылайте `Authorization: Bearer <volunteer JWT>` — тогда МНО
  привяжется к волонтёру и попадёт в `GET /volunteer/mno`. Токен опционален (без него — аноним, в «Мои» не попадёт).
- **Тело — `multipart/form-data`** (⚠️ НЕ JSON: несёт файлы фото). Поля-строки:
  `address`, `coords`, `name`?, `region_code`?, `city`?, `comment`?, `website`? (honeypot, пустое)
  — `address` и `coords` **обязательны** (иначе `400 VALIDATION_ERROR`).
- **`comment`** — необязательный комментарий волонтёра, текст **≤500** символов (обрезается сервером).
- **`photos`** — необязательный список файлов фото нового МНО (part с именем `photos`, до **3**,
  jpg/png/webp, ≤10 МБ каждый; сервер пере-кодирует в JPEG и делает превью). Невалидное фото
  (тип/размер/>3) → `400 VALIDATION_ERROR` (до записи). Это те же правила, что у `photos[]` в `POST /intake/form`.
- **200:** созданный МНО (`{ id, name, address, city, coords, reg:"", source:"volunteer",
  comment, photo_urls:[…], … }`). `id` кладите в `mno_id` отчёта (`POST /intake/form`), `reg` в `mno_reg`.
- `comment`/`photo_urls` есть только у волонтёрских МНО (у ФГИС/ручных — `null`/`[]`). URL из `photo_urls`
  отдаются публично через `GET /intake/mno-photo/{mno_id}/{filename}` (см. §8, анти-traversal как у фото обращений).
- Такой МНО помечается `source:"volunteer"` — в админке эколог видит бейдж «Добавлен волонтёром» и проверяет его.
...
### GET /intake/mno-photo/{mno_id}/{filename}
Публичная отдача байтов фото **волонтёрского МНО** (зеркалит `GET /intake/photo/…`, но для МНО).

- **Авторизация:** не требуется (публично).
- **Path:** `mno_id` — UUID МНО; `filename` — строго `^[0-9]+(_thumb)?\.jpg$`
  (полноразмер `0.jpg`… / превью `0_thumb.jpg`…).
- **Ответ 200:** тело — `image/jpeg` (`Content-Disposition: inline`, `X-Content-Type-Options: nosniff`).
- **Загрузка** фото МНО — внутри `multipart POST /intake/mno` (поле `photos`); этот эндпоинт только
  отдаёт сохранённое. URL-ы приходят в `photo_urls` карточки МНО. Анти-traversal как у фото обращений.
