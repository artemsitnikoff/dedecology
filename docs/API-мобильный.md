...
- **Тело:** `{ "address": "…", "coords": "55.0, 82.9", "name"?: "", "region_code"?: "54", "city"?: "", "website"?: "" }`
  — `address` и `coords` **обязательны** (иначе `400 VALIDATION_ERROR`); `website` — honeypot (пустое).
- **200:** созданный МНО (`{ id, name, address, city, coords, reg:"", source:"volunteer", … }`).
  `id` кладите в `mno_id` отчёта (`POST /intake/form`), `reg` в `mno_reg`.
