...
> **Письма (важно):** SMTP пока не подключён, поэтому `register` и `password/reset-request`
> возвращают токен/ссылку **прямо в ответе** (`email_sent: false` + `email_verify_token`/`reset_token`)
> — для интеграции и тестов. Когда подключат SMTP, письмо уйдёт на почту, а токен в ответе исчезнет
> (`email_sent: true`). Приложение должно работать в обоих режимах: `email_sent=false` → взять токен
> из ответа; `true` → ждать перехода по ссылке из письма.

### POST /volunteer/register
Регистрация волонтёра. **Публичный.**
- **Тело:** `{ "fio": "Иванов И.И.", "email": "v@mail.ru", "password": "≥6 симв." }`
- **200:** `{ "volunteer_id", "email", "email_sent": false, "email_verify_token": "...", "verify_url": "..." }` (токен/url — только при `email_sent=false`). Дубль email → `409 CONFLICT`.

### POST /volunteer/verify-email
Подтверждение почты по токену. **Публичный.**
- **Тело:** `{ "token": "..." }` → **200** `{ "ok": true, "email_verified": true }`. Битый/протухший → `400`.

### POST /volunteer/login
Вход. **Публичный.**
- **Тело:** `{ "email", "password" }`
- **200:** `{ "access_token", "token_type": "bearer", "volunteer": { "id","fio","email","phone","email_verified" } }`.
...
- **200:** `{ "id","fio","email","phone","email_verified" }`.
